...
func TestStore_Implements(t *testing.T) {
	var store interface{} = &Store{}
	if _, ok := store.(raft.StableStore); !ok {
		t.Fatalf("Store does not implement raft.StableStore")
	}
	if _, ok := store.(raft.LogStore); !ok {
		t.Fatalf("Store does not implement raft.LogStore")
	}
}
...
func TestStore_DeleteRange(t *testing.T) {
	...
	if err := store.GetLog(2, new(raft.Log)); err == nil {
